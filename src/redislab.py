...
class Redis_Client():
    ...
    That is, we expect to only search through the subset of the keyspace, and then incrementally iterate the next keyspace only if needed. 
    (https://redis.io/commands/scan/). You can test the scan query in the redis-cli.
    """
    def query3(self):
        print("Executing query 3.")
        # TODO:Get the keys and last names of the users whose ids do not start with an odd number. Searching for the keyspace start at cursor 1280.
    ...
    def query4(self):
        ...
        # TODO: In order to query attributes other than the primary key, you need to first create a secondary index in Redis with the following specification: 
